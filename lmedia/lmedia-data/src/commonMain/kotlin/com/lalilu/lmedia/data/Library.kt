...
abstract class Library {
    ...
    suspend fun getByPrefix(mediaId: String): LItem? {
        return when {
            mediaId.startsWith(LArtist.ID_PREFIX) -> get<LArtist>(mediaId.substringAfter(LArtist.ID_PREFIX))
            mediaId.startsWith(LAlbum.ID_PREFIX) -> get<LAlbum>(mediaId.substringAfter(LAlbum.ID_PREFIX))
            mediaId.startsWith(LGenre.ID_PREFIX) -> get<LGenre>(mediaId.substringAfter(LGenre.ID_PREFIX))
            mediaId.startsWith(LFolder.ID_PREFIX) -> get<LFolder>(mediaId.substringAfter(LFolder.ID_PREFIX))
            mediaId.startsWith(LAudio.ID_PREFIX) -> get<LAudio>(mediaId.substringAfter(LAudio.ID_PREFIX))
            else -> get<LAudio>(mediaId)
        }
    }

    suspend fun mapByByPrefix(ids: List<String>): List<LItem> = withContext(Dispatchers.io) {
        ids.map { mediaId -> async { getByPrefix(mediaId) } }
            .awaitAll()
            .filterNotNull()
    }
}