 ...
 * 将指定的分组项链接到当前对象
 *
 * @param item 要链接的分组项，必须是 Linkable 的实现
 * @param T 分组项的具体类型，必须实现自 Linkable
 */
inline fun <reified T : Linkable> T.link(item: T) {
 ...
 * 获取当前对象关联的指定类型的分组项列表
 *
 * @param T 要获取的分组项类型，必须实现自 Linkable
 * @return 包含所有关联的指定类型分组项的列表，如果没有则返回空列表
 */
inline fun <reified T : Linkable> T.ref(): List<T> {
 ...
 * 获取当前对象关联的指定类型的分组项数量
 *
 * @param T 要统计的分组项类型，必须实现自 Linkable
 * @return 关联的指定类型分组项的数量，如果没有则返回 0
 */
inline fun <reified T : Linkable> T.refCount(): Int {
